...
Way to request an address: Accessing the server address `api.ubiex.co` such as `api.ubiex.co/trade/api/v1/order`.
...
`https://api.ubiex.co/trade/api/v1/getOrder?accesskey={AccessKey}&market={Market}&nonce={Timestamp}&id={OrderId}&signature={Signature}`
